...
### K-means Clustering
- **Purpose:** K-means is a type of clustering that groups observations, the information provided in the rows of datasets, into clusters based on finding the best centroid positions. K-means and clustering generally are helpful for finding patterns and creating groups in the data.
...
- **Evaluation:** To help the user evaluate what the optimal number of clusters is, visualizations are created showing the silhouette score and the elbow method. ***Note*** while the Silhouette score is less subjective, there is no perfect metric, instead multiple metrics should be considered. Especially since unsupervised learning is part of exploratory data analysis, it is important to test different options and see which is best.
...
<img width="774" alt="Screenshot 2025-05-08 at 11 41 26 AM" src="https://github.com/user-attachments/assets/b142596f-fe9c-40f7-acfd-e151bd038da2" />
  ...
  - ***Linkage***: Linkage provides different ways to measure clusters before merging. Users are given the option of selecting one of the four linkage types described below:
     - *Complete:* When comparing two clusters, the complete linkage option will look for the *maximum distance between two points, one in each cluster. The drawback for complete linkage is a strong influence of outliers since it relies on maximum distances.
...
- ***Truncation***: Users can choose to truncate the dendrogram, which will group things together at a higher level and create a simpler dendrogram.
